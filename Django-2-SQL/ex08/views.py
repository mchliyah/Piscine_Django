    ...
    try:
        ...
        try:
            with open('d05_re/planets.csv', 'r') as f:
                cursor.copy_expert("COPY ex08_planets(name, climate, diameter, orbital_period, population, rotation_period, surface_water, terrain) FROM STDIN WITH (DELIMITER E'\\t', NULL 'NULL')", f)
            conn.commit()
            success_messages.append("OK")
        except Exception as e:
            conn.rollback()
        ...
        try:
            with open('d05_re/people.csv', 'r') as f:
                cursor.copy_expert("COPY ex08_people(name, birth_year, gender, eye_color, hair_color, height, mass, homeworld) FROM STDIN WITH (DELIMITER E'\\t', NULL 'NULL')", f)
            conn.commit()
            success_messages.append("OK")
        except Exception as e:
            conn.rollback()
